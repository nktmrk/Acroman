...
Meteor.methods({
  joinOrLeaveRoom(lobbyId, join) {
    ...
    if (!lobby || !lobby.official) throw new Meteor.Error("No valid room");
    ...
    if (join) {
      ...
      //LobbyManager.addSystemMessage(lobbyId, username + " joined.");
      //LobbyManager.addSystemMessage(null, username + " joined.");
      //LobbyManager.addSystemMessage(null, username + " joined the room " + lobby.displayName);
      ...
      username = displayName(this.userId, true);
      //LobbyManager.addSystemMessage( null, username + " left the room " + lobby.displayName );
      //LobbyManager.addSystemMessage( lobbyId, username + " has left.");
      //LobbyManager.addSystemMessage( null, username + " has left.");
    }
  },
  ...
  inviteToPlay(userId, lobbyId) {
    ...
    if (!lobby) throw new Meteor.Error("lobby-not-found", "Room not found");
...
if (Meteor.isServer) {
  // Rate limit joining or leaving a room to once every 5 seconds
  DDPRateLimiter.addRule(
    {
      name(name) {
        return name === "joinOrLeaveRoom";
      },
